...
> **版本引用**：生产环境使用 **`@v1.0.0`** 或 commit SHA；`@main` 仅用于跟踪最新开发版。
...
### 第 2 步：配置 Telegram（企业级 — Secrets 优先）

> **v1.0.0 起**：`config/telegram.yaml` **不含 Token**，详见 [SECURITY.md](SECURITY.md)。

**推荐（生产）**：在 GitHub `Settings → Secrets` 配置 `TG_BOT_TOKEN`、`TG_CHAT_ID`。

**本地开发**：复制 [`config/telegram.yaml.example`](config/telegram.yaml.example) → `config/telegram.local.yaml`（已 gitignore）。
...
jobs:
  audit:
    ...
    steps:
      ...
      - name: 运行代码审计 Skill
        uses: YOUR_USERNAME/code-audit-skill@v1.0.0   # 生产锁定版本
        with:
          working-directory: ${{ github.event.inputs.working-directory || '.' }}
          audit-preset: security                      # 企业预设：full|minimal|security|python|ci-fast
...
| [docs/GUIDANCE.md](docs/GUIDANCE.md) | 负责人 | **可行性评估与扩展指导意见** |
| [docs/LOCAL-RUN.md](docs/LOCAL-RUN.md) | 开发者 | **本地运行 Skill**（Git Bash/WSL/一键脚本） |
...
- [docs/GUIDANCE.md](docs/GUIDANCE.md) — 可行性评估与扩展指导意见
- [docs/LOCAL-RUN.md](docs/LOCAL-RUN.md) — 本地运行指南（`bash scripts/run-local-audit.sh`）
- [web/README.md](web/README.md) — **Web 控制台**（Docker 前后端、GitHub 选仓审计、文档下载/TG）
